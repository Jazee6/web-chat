...
import workletUrl from "@/lib/noise-suppressor-worklet.esm.js?url";
...
class NoiseSuppressionEffect {
  ...
  startEffect(audioStream: MediaStream): MediaStream {
    ...
    this._audioContext.audioWorklet
      ...
      .then(() => {
        ...
        if (!this._audioSource || !this._audioDestination) {
          throw new Error("Audio source or destination is not initialized");
        }
        this._audioSource
  ...
  stopEffect(): void {
    ...
    if (this._originalMediaTrack && this._outputMediaTrack) {
